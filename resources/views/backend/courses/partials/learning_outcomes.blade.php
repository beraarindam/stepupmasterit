...
<div class="mt-8 pt-6 border-t border-gray-100">
    ...
    <div class="space-y-3" x-data="courseLearningOutcomes({{ $learningOutcomesJs }})">
        <template x-for="(item, index) in items" x-bind:key="index">
            <div class="flex gap-3 items-center">
                <span class="text-gray-400 text-sm w-6 shrink-0" x-text="index + 1"></span>
                <input type="text" :name="'learning_outcomes[' + index + '][text]'" x-model="item.text"
                ...
                <button type="button" x-on:click="removeRow(index)"
                    class="shrink-0 px-3 py-2 text-sm font-medium text-red-600 hover:text-red-700 hover:bg-red-50 rounded-lg border border-transparent hover:border-red-100 transition">
...
<script>
    function courseLearningOutcomes(initialItems) {
